...
* Include the following below or next to each frame:
   * Scene/Shot Number
   * Shot Type (for example, close-up, wide shot, POV)
   * Camera Movement, if any (for example, pan, zoom, handheld)
...
* Mark how each shot transitions to the next (for example, cut, dissolve, fade).
